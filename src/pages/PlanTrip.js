...
export default function PlanTrip() {
  ...
  const handleSave = (e) => {
    ...
    const url = `${API_BASE}/trips/${tripId}/points${
      editingId ? `/${editingId}` : ""
    }`;
  ...
  useEffect(() => {
    ...
    const coords = displayPoints
      .map((p) => `${p.position.lng},${p.position.lat}`)
      .join(";");
    fetch(
  ...
  useEffect(() => {
    ...
    const bounds = L.latLngBounds(
      displayPoints.map((p) => [p.position.lat, p.position.lng])
    );
        ...
        <Col md={5} className="mt-3">
          <Form.Group className="mb-3">
            <div className="d-flex justify-content-end mb-2">
              <Button variant="success" onClick={() => setShowAutoModal(true)}>
            ...
            >
              {displayPoints.length === 0 ? (
                ...
                displayPoints.map((pt, idx) => (
                  ...
                  >
                    <Card
                      className={`shadow-sm w-75 ${
                        new Date(pt.date) < new Date(today)
                          ? "bg-light text-muted"
                          : ""
                      }`}
        ...
        <Col md={7}>
          <div style={{ height: "450px" }} className="mb-4">
            ...
            >
              ...
              {displayPoints.map((pt) => (
                <Marker
                  key={pt.id}
                  position={[pt.position.lat, pt.position.lng]}
                >
                  <Popup>
          ...
          {filterDate && routeDistance != null && (
            <p className="text-center mt-3">
              Trasa do przebycia na dzień{" "}
              <strong>
                {new Date(filterDate).toLocaleDateString("pl-PL")}
              </strong>{" "}
              ma długość:{" "}
              <strong>{(routeDistance / 1000).toFixed(2)} km</strong>
            </p>
          )}
        </Col>
      </Row>
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Form onSubmit={handleSave}>
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" type="submit" disabled={loadingName}>
              Zapisz
            </Button>
          </Modal.Footer>
        </Form>
          ...
          <Form>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                value={autoForm.city}
                onChange={(e) =>
                  setAutoForm({ ...autoForm, city: e.target.value })
                }
              />
            </Form.Group>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                min={today}
                onChange={(e) =>
                  setAutoForm({ ...autoForm, dateFrom: e.target.value })
                }
              />
            </Form.Group>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setAutoForm({ ...autoForm, dateTo: e.target.value })
                }
              />
            </Form.Group>
          </Form>
