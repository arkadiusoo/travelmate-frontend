...
export default function PlanTrip() {
    ...
    useEffect(() => {fetch(`${API_BASE}/notes/trip/${tripId}`, {
        headers: getAuthHeaders(),
        method: 'Get'
                                        ...
                                        <Card.Body className="d-flex">
                                            ...
                                            <div style={{
                                                ...
                                                <Button variant="outline-danger" size="sm"
                                                        onClick={e => handleRemove(e, pt.id)} title="Usuń">
                                                    <BsTrash/>
                                                </Button>
                                                <Button variant="outline-success" size="sm"
                                                        title="Oznacz jako odwiedzone" onClick={e => {
                                                    e.stopPropagation();
                                                    handleMarkVisited(pt.id);
                                                }} disabled={pt.visited}>
                                                    <BsCheckCircle/>
                                                </Button>
                                                <Button variant="outline-warning" size="sm" style={{minWidth: '30px'}}
                                                        onClick={() => openNoteModal(pt.id)} title="Notatki">
                                                    <BsJournalText/>
                                                </Button>
                                            </div>
                                        </Card.Body>
                ...
                <Col md={7}>
                    ...
                    {filterDate && routeDistance != null && (
                        <p className="text-center mt-3">
                            Trasa do przebycia na
                            dzień <strong>{new Date(filterDate).toLocaleDateString('pl-PL')}</strong> ma
                            długość: <strong>{(routeDistance / 1000).toFixed(2)} km</strong>
                        </p>
                    )}
                </Col>
            </Row>
